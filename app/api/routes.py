...
async def export_bot_docx(data: ExportRequest):
    ...
    try:
        ...
        task_type = task.get("task_type")
        if task_type != "bot_check":
            return {"error": f"Unsupported task type for bot DOCX export: {task_type}"}
        ...
        filepath = docx_generator.generate_bot_report(task_id, report_payload)
        if not filepath or not os.path.exists(filepath):
            return {"error": "Failed to generate report"}
        ...
        domain = re.sub(r"[^a-zA-Z0-9._-]+", "_", (urlparse(url).netloc or "site"))
        timestamp = datetime.utcnow().strftime("%Y-%m-%d_%H-%M")
        filename = f"bot_report_{domain}_{timestamp}.docx"
        return Response(
            content=content,
            media_type="application/vnd.openxmlformats-officedocument.wordprocessingml.document",
            headers={"Content-Disposition": f"attachment; filename={filename}"},
...
async def export_link_profile_docx(data: ExportRequest):
    ...
    try:
        ...
        task_type = task.get("task_type")
        if task_type != "link_profile_audit":
            return {"error": f"Unsupported task type for link profile DOCX export: {task_type}"}
        ...
        filepath = docx_generator.generate_link_profile_report(task_id, report_payload)
        if not filepath or not os.path.exists(filepath):
            return {"error": "Failed to generate report"}
        append_task_artifact(task_id, filepath, kind="export")
        ...
        timestamp = datetime.utcnow().strftime("%Y-%m-%d_%H-%M")
        filename = f"link_profile_report_{domain}_{timestamp}.docx"
        return Response(
            content=content,
            media_type="application/vnd.openxmlformats-officedocument.wordprocessingml.document",
            headers={"Content-Disposition": f"attachment; filename={filename}"},
...
async def export_link_profile_xlsx(data: ExportRequest):
    ...
    try:
        ...
        task_type = task.get("task_type")
        if task_type != "link_profile_audit":
            return {"error": f"Unsupported task type for link profile XLSX export: {task_type}"}
        ...
        filepath = xlsx_generator.generate_link_profile_report(task_id, report_payload)
        if not filepath or not os.path.exists(filepath):
            return {"error": "Failed to generate report"}
        append_task_artifact(task_id, filepath, kind="export")
        ...
        domain = re.sub(r"[^a-zA-Z0-9._-]+", "_", (urlparse(url).netloc or str(url) or "site"))
        timestamp = datetime.utcnow().strftime("%Y-%m-%d_%H-%M")
        filename = f"link_profile_report_{domain}_{timestamp}.xlsx"
        return Response(
            content=content,
            media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            headers={"Content-Disposition": f"attachment; filename={filename}"},
...
def _parse_clusterizer_keyword_line(raw_line: str) -> List[Dict[str, Any]]:
    ...
    line_unquoted = line.strip("\"'")
    if re.fullmatch(r"[+-]?\d+(?:[.,][0-9]+)?", line_unquoted):
        return []

    # Handle quoted multiline copy-paste blocks: "keyword\n123"
    if "\n" in line:
        block = line
        ...
        if len(parts) >= 2:
            freq = _parse_clusterizer_frequency(parts[-1])
...
def _collect_clusterizer_keyword_rows(keywords: Optional[List[str]], keywords_text: Optional[str]) -> List[Dict[str, Any]]:
    ...
    if raw_text.strip():
        for line in _split_clusterizer_text_rows(raw_text):
